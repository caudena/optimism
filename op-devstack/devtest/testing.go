...
// testP wraps a T to satisfy the P interface.
// This allows test-scoped code (devtest.T) to act as a package-level sponsor (devtest.P)
// when creating per-test orchestrators.
type testP struct {
	t T
}
...
// NewPFromT creates a P backed by a test T.
// Cleanup registered on the P is delegated to t.Cleanup.
// Close is a no-op since the T handles its own cleanup lifecycle.
func NewPFromT(t T) P {
	return &testP{t: t}
}
...
func (p *testP) Logf(format string, args ...any)   { p.t.Logf(format, args...) }
