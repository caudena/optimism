...
// ExtensibleComponentRegistry extends ComponentRegistry with mutation methods.
// Used during test setup to add components to containers.
type ExtensibleComponentRegistry interface {
	ComponentRegistry

	// AddComponent registers a component with the given ID.
	// Panics if a component with that ID already exists.
	AddComponent(id ComponentID, component any)
}
